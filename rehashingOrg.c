...
void main() {
	...
	int* tempTable = (int*)malloc(size * sizeof(int));
	int LOAD_BALANCE_LIMIT = (size * 0.75);
	initializeHashTable(hashTable, size);
    initializeHashTable(tempTable, size);
...
int rehashing(int* hashTable, int* tempTable, int size) {
	...
	size *= 2;
	hashTable = realloc(hashTable, size * sizeof(int));
	for (int i = 0; i < size; i++) {
		hashTable[i] = -1;
	}
    for (int i = 0; i < size / 2; i++) {
        if (tempTable[i] != -1) {
			if (insertWithLinearProbing(hashTable, size, tempTable[i])) {}
        }
    }
	for (int i = 0; i < size; i++) {
		printf("%d ", hashTable[i]);
...
void initializeHashTable(int* table, int size) {
	for (int i = 0; i < size; i++) {
		table[i] = -1;
	}
}
